...
const calculateProfileCompletion = (profile, userSkills) => {
    ...
    if (profile.experience && profile.experience.length > 0) {
        const completeExperiences = profile.experience.filter(exp => exp.title && exp.company && exp.startDate &&
            (exp.isCurrent || exp.endDate) &&
            exp.description && exp.description.trim().length > 10).length;
    ...
    if (profile.education && profile.education.length > 0) {
        const completeEducation = profile.education.filter(edu => edu.institution && edu.degree && edu.fieldOfStudy && edu.startDate).length;
...
const getProfileCompletionInsights = (profile, userSkills) => {
    const insights = [];
...
export const deleteEducation = async (req, res) => {
    try {
        ...
        const educationToDelete = profile.education.find(edu => edu._id?.toString() === educationId);
...
const getSkillRecommendations = async (userSkills, profile) => {
    try {
...
const getProfileStrengthBreakdown = (profile, userSkills) => {
    return {
...
const getProfileStrengths = (profile, userSkills) => {
    const strengths = [];
...
const getRecommendedNextSteps = (profile, userSkills) => {
    const steps = [];
    if (userSkills.length < 5) {
