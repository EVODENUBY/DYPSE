...
import { 
  ...
  enhancedExperienceValidation, 
  enhancedEducationValidation,
...
router.get('/skills/search', 
  query('q').notEmpty().trim().isLength({ min: 1, max: 50 }).withMessage('Search query is required'),
  searchSkills
);
...
router.delete('/skills/:skillId', 
  param('skillId').isMongoId().withMessage('Valid skill ID is required'),
  deleteSkill
);
...
router.put('/experience/:experienceId', 
  param('experienceId').isMongoId().withMessage('Valid experience ID is required'),
  enhancedExperienceValidation, 
  handleValidationErrors,
  updateExperience
);
...
router.put('/education/:educationId',
  param('educationId').isMongoId().withMessage('Valid education ID is required'),
  enhancedEducationValidation,
  handleValidationErrors,
  updateEducation
);
